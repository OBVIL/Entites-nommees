...
if ($ARGV[0]) {
	...
	@files = glob("../evaluation/L3i_NERC-EL/comparer-predictions-et-gold/*");
}
...
foreach (@files) {
	...
	my $gold_dir = "$directory" . "Gold_$work";

	my @golds = glob("$gold_dir/*");
	...
	for my $gold (@golds) {
		...
		$outfile = "$directory"."Predictions_vs_Gold_$work/TSV_" . $outfile;
